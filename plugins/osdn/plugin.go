...
func (plugin *OsdnNode) getVNID(namespace string) (string, error) {
	if plugin.multitenant {
		vnid, err := plugin.vnidMap.WaitAndGetVNID(namespace)
		if err != nil {
			return "", err
		}
...
func (plugin *OsdnNode) SetUpPod(namespace string, name string, id kubeletTypes.DockerID) error {
...
func (plugin *OsdnNode) TearDownPod(namespace string, name string, id kubeletTypes.DockerID) error {
...
func (plugin *OsdnNode) Status(namespace string, name string, id kubeletTypes.DockerID) (*knetwork.PodNetworkStatus, error) {
...
func (plugin *OsdnNode) vethSetup(id kubeletTypes.DockerID) (string, string, string, error) {
	containerNs, err := plugin.getContainerNS(id)
	if err != nil {
		return "", "", "", fmt.Errorf("Failed to get container network namespace: %v", err)
	}
...
// Returns host docker host veth name, container MAC address, and container IP address
func (plugin *OsdnNode) vethInspect(id kubeletTypes.DockerID) (string, string, string, error) {
	containerNs, err := plugin.getContainerNS(id)
	if err != nil {
		return "", "", "", fmt.Errorf("Failed to get container network namespace: %v", err)
	}
...
func (plugin *OsdnNode) getContainerNS(id kubeletTypes.DockerID) (ns.NetNS, error) {
...
func (plugin *OsdnNode) runIPAM(containerNs ns.NetNS, action string, id kubeletTypes.DockerID) (string, error) {
	if plugin.cniConfig == nil {
		_, ipnet, err := net.ParseCIDR(plugin.localSubnet.Subnet)
		if err != nil {
			return "", fmt.Errorf("Failed to parse node subnet: %v", err)
		}
		localSubnetGateway := netutils.GenerateDefaultGateway(ipnet).String()

		clusterNetwork, err := plugin.registry.GetClusterNetwork()
		if err != nil {
			return "", fmt.Errorf("Failed to get cluster network: %v", err)
		}

		plugin.cniConfig = []byte(fmt.Sprintf(cniConfigTemplate, plugin.localSubnet.Subnet, localSubnetGateway, clusterNetwork.String()))
	}
