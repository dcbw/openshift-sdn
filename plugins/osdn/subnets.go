...
func (node *OsdnNode) SubnetStartNode(mtu uint) (bool, error) {
	...
	clusterNetwork, _, servicesNetwork, err := node.registry.GetNetworkInfo()
	if err != nil {
		log.Errorf("Failed to obtain ClusterNetwork: %v", err)
		return false, err
	}
	networkChanged, err := node.SetupSDN(node.localSubnet.Subnet, clusterNetwork.String(), servicesNetwork.String(), mtu)
	if err != nil {
		return false, err
	}
